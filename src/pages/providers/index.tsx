...
function ProviderListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProviderInterface[]>(
    ...
    () =>
      getProviders({
        relations: ['user', 'feedback.count', 'lesson.count', 'match.count', 'puzzle.count', 'strategy_guide.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('puzzle', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>puzzle</Th>}
                  {hasAccess('strategy_guide', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>strategy_guide</Th>
                  )}
                  <Th>Actions</Th>
                </Tr>
              </Thead>
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('puzzle', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.puzzle}</Td>
                    )}
                    {hasAccess('strategy_guide', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.strategy_guide}</Td>
                    )}
                    <Td>
