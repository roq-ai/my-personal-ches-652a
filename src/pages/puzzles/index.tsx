...
import { getPuzzles, deletePuzzleById } from 'apiSdk/puzzles';
...
function PuzzleListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('puzzle', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/puzzles/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('puzzle', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/puzzles/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('puzzle', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/puzzles/edit/${record.id}`} passHref legacyBehavior>
                          <Button
